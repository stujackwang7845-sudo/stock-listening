    ...
    def run(self):
        ...
        for code, info in disposition_map.items():
            if code in agg_data:
                ...
                agg_data[code]["newly_announced"] = info.get("newly_announced", False)
            else:
                ...
                agg_data[code] = {
                    ...
                    "newly_announced": info.get("newly_announced", False)
                }
...
class Dashboard(QWidget):
    ...
    def highlight_stock(self, code):
        ...
        if found:
            ...
            # Start Worker
            self.calc_worker = CalculationWorker(code, source, target_date=self.current_display_date)
...
# --- Worker ---
    ...
    def on_data_ready(self, agg_data):
        ...
        self.initial_load_finished.emit() # Signal Loading Done
    ...
    def populate_table(self, agg_data):
        ...
        try:
             # Use current display date
             t_date = self.current_display_date
             att_list = AttentionScraper.fetch_data(t_date)
             self.today_attention_list = att_list # Store list for Observer Box
             self.today_attention_map = {item['code']: item['reason'] for item in att_list}
             self.today_attention_names = {item['code']: item['name'] for item in att_list}
        except Exception as e:
             print(f"Scraper Init Error: {e}")
        ...
        if twse_disp:
            parsed = parser.parse_twse_disposition(twse_disp)
            for item in parsed:
                announce_date = item.get("announce_date", "").replace("/", "")  # 移除斜線: 115/01/06 -> 1150106
                print(f"DEBUG TWSE: {item['code']} {item['name']} | announce_date='{announce_date}'")
                if announce_date:
                    all_announce_dates.add(announce_date)
                    item["announce_date"] = announce_date  # 更新為統一格式
        ...
        if tpex_disp:
            parsed = parser.parse_tpex_disposition(tpex_disp)
            for item in parsed:
                announce_date = item.get("announce_date", "").replace("/", "")  # 確保無斜線
                print(f"DEBUG TPEX: {item['code']} {item['name']} | announce_date='{announce_date}'")
                if announce_date:
                    all_announce_dates.add(announce_date)
                    item["announce_date"] = announce_date  # 更新為統一格式
                    all_items.append(item)
        ...
        # 找出最新的公告日期
        latest_announce_date = max(all_announce_dates) if all_announce_dates else ""
        
        # 只取最新公告日期的股票代碼
        today_disposition_codes = set()
        for item in all_items:
            if item.get("announce_date") == latest_announce_date:
                today_disposition_codes.add(item["code"].strip())
        
        print(f"DEBUG: 最新公告日期: {latest_announce_date}, 處置股: {today_disposition_codes}")
        ...
        for code in sorted_codes:
            ...
            if data.get("is_disposed", False) and disp_start_dt:
                 ...
                 # 檢查是否為最新公告的處置股
                 if code in today_disposition_codes:
                     suffix = "(期)" if data.get("has_futures") else ""
                     
                     # 過濾可轉債 (上市+上櫃都過濾)
                     is_convertible_bond = (len(code) == 5) or any(marker in name for marker in ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十'])
                     
                     if not is_convertible_bond:
                         item_str = f"<a href='{code}' style='color: #E0E0E0; text-decoration: none;'>{code}&nbsp;{name}{suffix}</a>"
                         if data["source"] == "上市": 
                             notice_twse.append(item_str)
                         else:  # 上櫃
                             notice_tpex.append(item_str)
        ...
        for item in self.today_attention_list:
            ...
            # Check for Futures
            has_futures = False
            if c in agg_data and agg_data[c].get('has_futures'):
                has_futures = True
            ...
            if src == "上市":
                listening_twse.append(link)
            else:
                listening_tpex.append(link) # Default to OTC if unknown or marked
                
        # 2. One Step (From Calculation Loop)
        # (one_step_twse/tpex lists were populated in the loop)
        ...
        # [Cache Agg Data for fast startup]
...
class CalculationWorker(QThread):
    ...
    def __init__(self, code, source, target_date=None):
        super().__init__()
        self.code = code
        self.source = source
