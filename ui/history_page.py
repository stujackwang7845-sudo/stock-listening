...
class HistoryPage(QWidget):
    ...
    def init_ui(self):
        ...
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents) # Status - Swapped
    ...
    def load_items(self):
        ...
        for r_idx, r in enumerate(filtered_records):
            ...
            t_widget = self._render_trigger_cell(trigger_info)
    ...
    def _render_trigger_cell(self, trigger_info):
        ...
        if isinstance(trigger_info, dict):
            ...
            try:
                ...
                return QLabel(str(trigger_info))
        
        if not raw_map:
             return QLabel("-")

        # 2. Date Processing (Year Rollover)
        parsed_dates = []
        now = datetime.now()
        ...
        # 3. Smart Trimming
        start_idx = 0
        for i, (dt_obj, val) in enumerate(parsed_dates):
            clean_v = val.strip()
            if clean_v and clean_v not in ["-", "—", "–"]:
                start_idx = i
                break
        
        trimmed_dates = parsed_dates[start_idx:]
        if not trimmed_dates:
             trimmed_dates = parsed_dates[-5:]

        min_date = trimmed_dates[0][0]
        max_date = parsed_dates[-1][0]
        ...
        while curr <= max_date:
            if DateUtils.is_trading_day(curr):
                ...
                val = lookup_map.get(k1, lookup_map.get(k2, "-"))
                final_list.append((curr, val))
            curr += timedelta(days=1)
            
        # [Limit] Max 9 days as per user request
        if len(final_list) > 9:
        ...
        for dt_obj, val in final_list:
            ...
            if "一" in v_str:
                color = "#FF4444"
            elif v_str and v_str not in ["-", "—", "–"]:
                color = "#4da6ff"
            
            clause_row += f"<td style='border: 1px solid #555; padding: 6px; font-weight: bold; font-size: 12px; color: {color};'>{v_str}</td>"
            
        date_row += "</tr>"
        clause_row += "</tr>"
        html += date_row + clause_row + "</table>"
