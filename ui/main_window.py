...
class MainWindow(QMainWindow):
    def __init__(self, auto_start=True):
        ...
        self.setWindowIcon(QIcon("prison.png")) # Explicitly set icon
    ...
    def init_ui(self):
        ...
        self.tab3 = HistoryPage(self.history_manager)
        self.tab4 = QWidget()
    ...
    def preload_data(self, status_callback, done_callback):
        """
        Triggers Dashboard data loading for Splash Screen.
        """
        # Connect status update
        self.tab1.status_message_updated.connect(status_callback)
        
        # Connect completion
        # We disconnect after first fire to ensure splash logic only runs once
        def one_shot_done():
            self.tab1.status_message_updated.disconnect(status_callback)
            self.tab1.initial_load_finished.disconnect(one_shot_done)
            done_callback()
            
        self.tab1.initial_load_finished.connect(one_shot_done)
        
        # Start
        self.tab1.start_worker(force_refresh=False)
