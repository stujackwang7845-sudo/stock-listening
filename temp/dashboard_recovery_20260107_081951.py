...
class HistoryWorker(QThread):
    ...
    def run(self):
        ...
        # TWSE Disposition
        twse_disp = fetcher.fetch_twse_disposition(margin_date_str)
        if twse_disp:
...
class Dashboard(QWidget):
    ...
    def init_ui(self):
        ...
        right_layout.addWidget(QLabel("進處置提條件:"))
    ...
    def update_headers(self):
        last_trading_day = DateUtils.get_last_trading_day()
        self.calendar = DateUtils.get_market_calendar(last_trading_day)
        
        headers = ["股票", "名稱", "類別", "融券", "期貨"]
        headers.extend(self.calendar["past"]) # 8 days
        headers.append(f"{self.calendar['current']} (今)") # 1 day
        headers.extend(self.calendar["future"]) # Future days (User set to 5 now)
    ...
    def populate_table(self, agg_data):
        ...
        date_cols = self.calendar["past"] + [self.calendar["current"]]
        
        # Prepare prediction dates (Need 10 days history ending today)
        pred_history_dates = []
        last_trading_day = DateUtils.get_last_trading_day()
        curr = last_trading_day
        count = 0
...
class Dashboard(QWidget):
    ...
    def populate_table(self, agg_data):
        ...
        for code in sorted_codes:
            ...
            warning_msg, prob = DispositionPredictor.analyze(hist_items, future_days=len(self.calendar["future"]))
        ...
        for row, (code, data, warning_msg, prob) in enumerate(final_rows):
            ...
            self.grid_table.setItem(row, 3, short_item)
            
            # Futures Status (Col 4)
            ...
            self.grid_table.setItem(row, 4, futures_item)
