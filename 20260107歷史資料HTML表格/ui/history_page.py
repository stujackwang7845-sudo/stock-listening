...
class HistoryPage(QWidget):
    ...
    def init_ui(self):
        ...
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.Fixed) # Trigger (Table)
        ...
        # Fixed width
        self.table.setColumnWidth(2, 420) # Trigger Table
    ...
    def load_items(self):
        from core.utils import DateUtils # 規格 4.2
        ...
        for r_idx, r in enumerate(filtered_records):
            ...
            # 2. Trigger Info (Swapped to Col 2, Table Spec)
            trigger_info = r.get('trigger_info', '')
            
            try:
                # 2.1 Parse JSON
                if isinstance(trigger_info, str) and trigger_info:
                    import json
                    clauses_map = json.loads(trigger_info)
                elif isinstance(trigger_info, dict):
                    clauses_map = trigger_info
                else:
                    clauses_map = {}
                    
                # 2.2 Date Processing
                history_dates_raw = list(clauses_map.keys())
                date_objects = []
                current_year = datetime.now().year
                
                for d_str in history_dates_raw:
                    try:
                        ...
                        if month > 6 and current_month <= 6:
                            year = current_year - 1
                        else:
                            year = current_year
                            
                        dt = datetime(year, month, day)
                        date_objects.append((d_str, dt))
                    except:
                        pass
                ...
                # 2.3 Smart Trimming & 2.4 Gap Filling
                final_dates = []
                if date_objects:
                    # Find start_idx
                    start_idx = 0
                    for i, (d_str, dt) in enumerate(date_objects):
                        ...
                        if val == "-":
                            short_str = f"{dt.month}/{dt.day}"
                            val = clauses_map.get(short_str, "-")
                        
                        if val and val != "-" and val.strip():
                            start_idx = i
                            break
                    
                    if start_idx >= len(date_objects): start_idx = 0
                    
                    min_date = date_objects[start_idx][1]
                    max_date = date_objects[-1][1]
                    ...
                    while curr <= max_date:
                        if DateUtils.is_trading_day(curr):
                            ...
                            val = clauses_map.get(d_str, "-")
                            if val == "-":
                                short_str = f"{curr.month}/{curr.day}"
                                val = clauses_map.get(short_str, "-")
                            final_dates.append((d_str, val))
                        curr += timedelta(days=1)
                else:
                    for k in sorted(clauses_map.keys()):
                        final_dates.append((k, clauses_map[k]))
                        
                # 3. UI HTML
                hist_html = "<table style='border-collapse: collapse; width: 100%; text-align: center; background-color: #1E1E1E;'>"
                
                # Header
                hist_html += "<tr>"
                for d_str, _ in final_dates:
                    hist_html += f"<th style='border: 1px solid #555; padding: 4px; color: #BBB; font-size: 11px;'>{d_str}</th>"
                hist_html += "</tr>"
                
                # Body
                hist_html += "<tr>"
                for _, clause in final_dates:
                    if "一" in str(clause): color = "#FF4444"
                    elif clause and clause != "-": color = "#4da6ff"
                    else: color = "#555"
                    
                    hist_html += f"<td style='border: 1px solid #555; padding: 6px; color: {color}; font-weight: bold; font-size: 12px;'>{clause}</td>"
                hist_html += "</tr></table>"
                
                table_label = QLabel(hist_html)
                table_label.setTextFormat(Qt.TextFormat.RichText)
                table_label.setStyleSheet("QLabel { background-color: #1E1E1E; }")
                self.table.setCellWidget(r_idx, 2, table_label)
                
            except Exception as e:
                # Fallback
                t_str = str(trigger_info)
                item_trig = QTableWidgetItem(t_str)
                item_trig.setToolTip(t_str)
